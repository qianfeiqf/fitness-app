...
Page({
  ...
  getTemplateIntro(templateId) {
    const intros = {
      ...
      'tpl_stronglifts': {
        background: 'StrongLifts 5×5由比利时健身教练Mehdi在2007年创立，基于Bill Starr的原始5×5框架，但进行了彻底的现代化改良。与原版5×5不同，StrongLifts采用AB交替模式——A日训练深蹲+卧推+划船，B日训练深蹲+硬拉+卧推。Mehdi的创新在于构建了一个完整的训练生态：配套App自动计算重量、记录组数、设置休息时间，并提供动作视频指导。其核心理念是"简化一切"——你不需要思考，只需要跟着计划走。全球已有数百万人使用StrongLifts App进行训练，是最受欢迎的入门力量训练计划之一。',
        suitable_for: '完全健身新手（尤其是男性）；希望以深蹲为训练核心的人群；需要一个清晰指导体系帮助入门的训练者；想要培养训练习惯但不知从何开始的初学者。时间充裕、愿意使用App辅助训练的人更能坚持该计划。',
        notes: '每次训练前务必完成5-10分钟肩袖群热身和髋关节活动度练习。起始重量：有训练基础者从5RM的50%开始；无基础者从空杆开始（深蹲/卧推/推举20kg，硬拉40kg，划船30kg）。加重原则：深蹲每次+2.5kg，卧推/推举/划船每次+1-2.5kg，硬拉到100kg后改为每次+2.5kg。如果A日深蹲感觉极度疲劳，B日的深蹲可降至70%重量完成。每周3次训练建议固定在周一/周三/周五，便于形成节奏。'
      },
      'tpl_starting': {
