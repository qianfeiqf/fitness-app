...
function updateFailureCount(exerciseId, count) {
  ...
  if (failureRecord) {
    // 使用 id || _id 兼容两种字段名
    update('failure_counts', failureRecord.id || failureRecord._id, { count });
  } else {
...
function getIncrementStep(planExercise, exercise) {
  ...
  if (profiles && profiles.length > 0) {
    // 使用最新的profile记录
    const latestProfile = profiles.reduce((latest, p) =>
      (p.updated_at > (latest?.updated_at || 0)) ? p : latest, null);
    if (latestProfile && latestProfile.weight) {
      bodyWeight = latestProfile.weight;
    }
  }
